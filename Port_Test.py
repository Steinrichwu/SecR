...
class Backtest():
    ...
    def Raisecon_EPS(self,dailyreturn,rebaldaylist):
        Postab=CP.Consensus('Raisecon_eps',rebaldaylist)
        Postab=Postab.groupby('date').apply(lambda x: x.nlargest(70,'sigvalue')).reset_index(drop=True)
        #Postab=self.BST.Intersect_weighting(Postab,'CSIAll')
        Postab=Postab.loc[Postab['ticker'].str[0:2]!='68',:].copy()               #排除掉科创板公司
...
class PortConst():
    ...
    def PartA_Shen7Top(self,dailyreturn,rebaldaylist):
        ...
        Markettop=fztab.groupby('date').apply(lambda x: x.nlargest(40,'meanscore')).reset_index(drop=True)
        Markettop=WS.Generate_PortNavEqual(Markettop)
        return(Markettop)
    ...
    def PartC_Consensus(self,dailyreturn,rebaldaylist):
        ...
        ConPostab=self.BT.Raisecon_EPS(dailyreturn,rebaldaylist)
    ...
    def PortBuild_Agressive(self,dailyreturn,rebaldaylist):
        MarkettopPos=self.PartA_Shen7Top(dailyreturn,rebaldaylist)
        ConsensusPos=self.PartC_Consensus(dailyreturn,rebaldaylist)
        Shen56CSIPos=self.PartD_Shen56CSI(dailyreturn,rebaldaylist)
        MarkettopPos['PortNav%']=MarkettopPos['PortNav%']*0.3 #0.3 (for presentation backtest)
        ...
        appended_port=MarkettopPos.append([ConsensusPos,Shen56CSIPos])
        mergedPos=self.BST.Merge_port(appended_port)
        mergedPosNew=self.SimpMerged(dailyreturn,mergedPos)
        mergedPosNew=self.Risk.RiskMgmt(mergedPosNew)
        return(mergedPosNew)
    
    def PortBuild_Test(self,dailyreturn,rebaldaylist):
        MarkettopPos=self.PartA_Shen7Top(dailyreturn,rebaldaylist)
        ConsensusPos=self.PartC_Consensus(dailyreturn,rebaldaylist)
        Shen56CSIPos=self.PartD_Shen56CSI(dailyreturn,rebaldaylist)
        return(MarkettopPos,ConsensusPos,Shen56CSIPos)
    ...
    def PortBuild_Original(self,dailyreturn,rebaldaylist):
        MarkettopPos=self.PartA_Shen7Top(dailyreturn,rebaldaylist)
        ConsensusPos=self.PartC_Consensus(dailyreturn,rebaldaylist)
        Shen56CSIPos=self.PartD_Shen56CSI(dailyreturn,rebaldaylist)
        MarkettopPos['PortNav%']=MarkettopPos['PortNav%']*0.3 #0.3 (for presentation backtest)
    ...
    def DailyModel(self,dailyreturn,rebalday):
        ...
        MarkettopPos['PortNav%']=MarkettopPos['PortNav%']*0.15
        ConsensusPos['PortNav%']=ConsensusPos['PortNav%']*0.35
        Shen56Pos['PortNav%']=Shen56Pos['PortNav%']*0.5
    ...
    def DailyModel_2021(self,dailyreturn,rebalday):
        today=date.today()
        todayname=str(today.strftime("%Y-%m-%d"))
        TAcount=pd.read_csv("D:/SecR/ThreeFourlatest.csv")
        TAcount['ticker']=[str(x).zfill(6)for x in TAcount['ticker']]
        Shen56CSIpos=self.BST.Intersect_weighting(TAcount,'CSI300')
        ConsensusPos=self.PartC_Consensus(dailyreturn,rebalday)
        mergedPos=Shen56CSIpos.append(ConsensusPos)
        mergedPos['PortNav%']=mergedPos['PortNav%']*0.5
        newtodayname=todayname.replace("-",'')
        mergedPos.to_csv("D:/CompanyData/ModelPortfolio_APort_"+newtodayname+".csv",index=False)
        return(mergedPos)