...
#Get the analyst_rolling P&L on rebaldays, rebaldays in one go
    def General_prep(self,lookback_period,rebaldaylist):
        ...
        sql=[]
        for rebalday in rebaldaylist:
            sqlpart="Select '"+rebalday+"' as date, MP.analyst_id, count(MP.date) as zerodays from MODEL_PORTFOLIO MP where ISNUMERIC(MP.analyst_id)=1 and MP.dtd_chg=0 and date>=dateadd(day,-"+str(lookback_period)+",'"+rebalday+"') and date<'"+rebalday+"' GROUP by MP.analyst_id"
            sql.append(sqlpart)
        reslist=ms.ExecListQuery(sql)
        analyst_zerodays=pd.DataFrame(reslist,columns=['date','analyst_id','zerodays'])
        analyst_pnl['index']=analyst_pnl['date']+analyst_pnl['analyst_id']
        analyst_zerodays['index']=analyst_zerodays['date']+analyst_zerodays['analyst_id']
        analyst_pnl=pd.merge(analyst_pnl,analyst_zerodays[['index','zerodays']],on='index',how='left')
        analyst_pnl=analyst_pnl.loc[analyst_pnl['zerodays']<(lookback_period*0.4)]    #Select the analsyts with 0 dailyPNL more than 60% of the days in lookback period
        analyst_pnl=analyst_pnl[['date','analyst_id','cumPNL']]
        return(analyst_pnl)
...
class Top_analyst():
    ...
    def Top_analyst_nonSector(self,dailyreturn,rebaldaylist,lookback_period,benchmark):
        ...
        for rebalday in rebaldaylist:
            ...
            analystnum=int(rebalday_analyst.shape[0]*0.3)
...
#downlaod the rolling 60day return and benchmark returns of analsyts
    def Analyst_history(self):
        tradingday=pd.read_csv("D:/SecR/Tradingday.csv")  #updated by DailyUpdate Tab, done together with Dailyreturn 
        ...
        df.to_csv("D:/SecR/analyst.csv",index=False)
        return(df)    
...
#Return the rank of all analysts' historical qmean of 60day rollingAlpha,The KeyFunction of Niu2
    def Analyst_rank_total(self):
        analyst=pd.read_csv("D:/SecR/analyst.csv")            #all analyst' 60day cumulativer return and 60day cumulative Benchmarket return              
