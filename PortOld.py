...
class Structuring():
    ...
    #Get rid of FIG stocks in the position table 
    def NoFinance(self,df):
        df=DC.Sector_get(df,'CSI')
        df=df.loc[df['primecode']!='06',:].copy()
        return(df)
...
class BySectorPort():
    ...
    def BySectorFacdict(self):
        dicfacdict={}
        dicfacdict['00']={'Quality': ['CashRateOfSales'],'Growth': ['QRevenuegrowth', 'QNetprofitgrowth'],'Value': ['Valuecon_pe'],'Market':['turnoverweek']}
...
class Backtest():
    ...
    def CrossSectorBT(self,dailyreturn,rebaldaylist):
        ...
        Postab=self.BST.Intersect_weighting(Postab,'CSI300')
...
class PortConst():
    ...
    def PartA_Shen7Top40(self,dailyreturn,rebaldaylist):
        ...
        #Markettop40=fztab.groupby('date').apply(lambda x: x.nlargest(40,'meanscore')).reset_index(drop=True)
        #Markettop40Pos=WS.Generate_PortNavEqual(Markettop40)
        Markettop=fztab.groupby('date').apply(lambda x: x.nlargest(100,'meanscore')).reset_index(drop=True) #100 for the presentation
        MarkettopPos=self.BST.Intersect_weighting(Markettop,'CSI300')
        #Markettop40Pos['memb']=Markettop40Pos.groupby('date')['ticker'].transform('count')           #Take intersect, but use equal weight
        #Markettop40Pos['PortNav%']=1/Markettop40Pos['memb']
        #Markettop40Pos=Markettop40Pos.drop(['memb'],axis=1)
        return(MarkettopPos)
    ...
    def PartD_Shen56CSIDaily(self,dailyreturn,rebaldaylist,TAcount):
    ...
    def PortBuild(self,dailyreturn,rebaldaylist):
        ...
        Markettop40Pos['PortNav%']=Markettop40Pos['PortNav%']*0.25 #0.3 (for presentation backtest)
        ...
        RPPos['PortNav%']=RPPos['PortNav%']*0.2 #Not allocated in presentation backtest
    ...
    def Execution(self):
        ...
        rebaldaylist=DC.Rebaldaylist(startdate,rebal_period)
        dailyreturn=DC.Dailyreturn_retrieve()
        mergedPos=self.PortBuild(dailyreturn,rebaldaylist)
        Pos=self.SimpMerged(dailyreturn,mergedPos)
        PNL=self.BT.GenPNL(Pos,dailyreturn)
        PNLreview=self.Alpha(PNL)
        return(Pos,PNL,PNLreview)
    
    def DailyModel(self,dailyreturn,rebalday,TAcount):
        today=date.today()
        todayname=str(today.strftime("%Y-%m-%d"))
        Markettop40Pos=self.PartA_Shen7Top40(dailyreturn,rebalday)
        BySectorSimPos=self.PartB_BySector(dailyreturn,rebalday)
        ConsensusPos=self.PartC_Consensus(dailyreturn,rebalday)
        Shen56Pos=self.PartD_Shen56CSIDaily(dailyreturn, rebalday, TAcount)
        Markettop40Pos,BySectorSimPos,ConsensusPos,Shen56Pos=map(lambda df: df.sort_values(by=['PortNav%'],ascending=False),[Markettop40Pos,BySectorSimPos,ConsensusPos,Shen56Pos])
        Markettop40Pos.to_csv("D:/CompanyData/Markettop40Pos_"+todayname+".csv",index=False)
        BySectorSimPos.to_csv("D:/CompanyData/BySectorSimPos_"+todayname+".csv",index=False)
        ...
        Markettop40Pos['PortNav%']=Markettop40Pos['PortNav%']*0.3
        BySectorSimPos['PortNav%']=BySectorSimPos['PortNav%']*0.15
        ConsensusPos['PortNav%']=ConsensusPos['PortNav%']*0.3
        Shen56Pos['PortNav%']=Shen56Pos['PortNav%']*0.25
        appended_port=Markettop40Pos.append([BySectorSimPos,ConsensusPos,Shen56Pos])
        mergedPos=self.BST.Merge_port(appended_port)
        mergedPosNew=self.SimpMerged(dailyreturn,mergedPos)
